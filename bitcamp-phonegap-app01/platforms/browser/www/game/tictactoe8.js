...
$('#cell-box').on('complete', (e, result) => {
    setTimeout(() => {
        if ( result == 1) alert("인간승리!");
        else if (result == -1) alert("컴퓨터승리!");
    }, 500);
});
...
$('#new-game').click((e) => {
		cells.text('').removeClass('cell-x').removeClass('cell-y');
		count =0;
		isCompleted =false;
		isWorking = false;
...
function checkCell(no, gamer) {
	$(cells[no]).text(
        ...
        .addClass(gamer == 'computer' ? 'cell-y':'cell-x');
}

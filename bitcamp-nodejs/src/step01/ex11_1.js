...
function require2(moduleName) {
  //1) npm 이 모듈들을 저장하는 폴더에 해당 이름의 모듈이 있는지 찾아본다.
  //   => 만약있다면 그 모듈 파일을 읽어 들여 실행한다.
  //2) 없다면, 파일 경로에서 해당 파일을 찾아 실행한다.
  //3) 모듈에서 exports 에 보관한 객체를 꺼내 따로 이름을 붙여 관리한다.
  //4) 그리고 그 객체를 리턴한다.
  ...
  if( !global.moduleMap[moduleName]){
    //해당 모듈이 없다면,
    // 모듈을 실행후 그 모듈이름으로 exports 값을 보관하다.
    ///=> exports 값을 보관했다고 가정하는 것이다.
